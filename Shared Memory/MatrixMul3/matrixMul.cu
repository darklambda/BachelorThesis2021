...
__global__ void MatrixMulCUDA(float *C, float *A, float *B, int wA, int wB, int wAt) {
  // Block index
  int bx = blockIdx.x;
  int by = blockIdx.y;
  ...
  int aEnd = aBegin + wAt * BLOCK_SIZE * ((wA / BLOCK_SIZE) - 1) + 1;

  // Step size used to iterate through the sub-matrices of A
  int aStep = BLOCK_SIZE * wAt;
...
int MatrixMultiply(int argc, char **argv, int block_size, const dim3 &dimsA,
  ...
  TransposeMatrix(h_A, size_A, dimsA.x, dimsA.y);
  ...
  if (block_size == 16) {
    MatrixMulCUDA<16>
        <<<grid, threads, 0, stream>>>(d_C, d_A, d_B, dimsA.x, dimsB.x, dimsA.y);
  } else {
    MatrixMulCUDA<32>
        <<<grid, threads, 0, stream>>>(d_C, d_A, d_B, dimsA.x, dimsB.x, dimsA.y);
  }
  ...
  for (int i = 0; i < static_cast<int>(dimsA.x); i++){
    Ctmp += h_A[i * static_cast<int>(dimsA.y)] * h_B[i * static_cast<int>(dimsB.x)];
  }
