...
/**
 * Matrix multiplication (CUDA Kernel) on the device: C = A * B, where B as parameter is the Bt, the traspose of B
 * wA is A's width and wB is B's width
 */
template <int BLOCK_SIZE>
__global__ void MatrixMulCUDA(float *C, float *A, float *B, int wA, int wB, int wBt) {
  // Block index
  int bx = blockIdx.x;
  int by = blockIdx.y;
  ...
  int bBegin = wBt * BLOCK_SIZE * bx; //cambio

  // Step size used to iterate through the sub-matrices of B
  int bStep = BLOCK_SIZE; //cambio
...
#pragma unroll
  ...
  int c = wB * BLOCK_SIZE * by + BLOCK_SIZE * bx; // inicio del bloque de cada thread (x*Block_Size,y*Block_Size) en terminos de c
  C[c + wB * ty + tx] = Csub;
}
...
int MatrixMultiply(int argc, char **argv, int block_size, const dim3 &dimsA,
  ...
  TransposeMatrix(h_B, size_B, dimsB.x, dimsB.y);
  ...
  if (block_size == 16) {
    MatrixMulCUDA<16>
        <<<grid, threads, 0, stream>>>(d_C, d_A, d_B, dimsA.x, dimsB.x, dimsB.y); //cambio
  } else {
    MatrixMulCUDA<32>
        <<<grid, threads, 0, stream>>>(d_C, d_A, d_B, dimsA.x, dimsB.x, dimsB.y); //cambio
  }
